...
model=$1
model_="\<${1}\>" # tv.. | catboost | ftt | mlp | dcnv2
reduce_data=$2    # any value [0,1)
task=$3           # either binclass | openml
param_name="n_vectors"
# param_ratio_list=("0.1" "0.3" "0.5" "0.7" "0.9")
param_ratio_list=("0.01" "0.05")
...
echo "For model: $model, reduce_data: $reduce_data, task: $task"
...
for DATASET in $iter; do
	...
	if [[ ${DNN[@]} =~ $model_ ]]; then
		for param_ratio in "${param_ratio_list[@]}"; do
			echo "  $param_name: $param_ratio"
			python scripts/parameter_effect/test_tuned_dnn.py \
				--model "$model" \
				--dataset "$file_name" \
				--task "$task" \
	...
	if [[ ${TREE[@]} =~ $model_ ]]; then
		for param_ratio in "${param_ratio_list[@]}"; do
			echo "  $param_name: $param_ratio"
			python scripts/parameter_effect/test_tuned_trees.py \
				--model "$model" \
				--dataset "$file_name" \
				--task "$task" \
				--reduce_data "$reduce_data" \
	...
	if [[ "$model" == "ftt" ]]; then
		for param_ratio in "${param_ratio_list[@]}"; do
			echo "  $param_name: $param_ratio"
			python scripts/parameter_effect/test_tuned_ftt.py \
				--dataset "$file_name" \
				--task "$task" \
				--reduce_data "$reduce_data" \
				--param_name "$param_name" \
