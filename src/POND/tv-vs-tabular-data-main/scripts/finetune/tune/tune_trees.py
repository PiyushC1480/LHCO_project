...
model_param_spaces = {
    'xgboost': {
        ...
        "tree_method": "hist"  # changed from gpu_hist to hist, for GPU device=cuda passed in model initializasiton
    },
    'catboost': {
        # base config
        'iterations': 2000,
...
def objective(trial):
    ...
    if args.model == 'xgboost':
        if dataset.is_regression:
            model = XGBRegressor(**cfg_model, device='cuda', random_state=42)
            predict = model.predict
        else:
            cfg_model['early_stopping_rounds'] = cfg_training['early_stopping_rounds']
            cfg_training.__delitem__('early_stopping_rounds')
            if dataset.is_multiclass:
                cfg_model['eval_metric'] = 'merror'
            else:
                cfg_model['eval_metric'] = 'error'

            model = XGBClassifier(**cfg_model, deivce='cuda', random_state=42, disable_default_eval_metric=True)
            if dataset.is_multiclass:
