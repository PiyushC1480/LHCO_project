...
model_param_spaces = {
    ...
    "tv7.1": {  ## Removed 0.5 from output and using hinge loss, 
        "n_vectors": (10, 500, 'int'),
        "n_kernel": (10, 300, 'int'),
        "kernel_std": (0.5, 1, 'float'),
        "use_kernel": (['rbf', 'lin'], 'categorical'),
    ...
    "tv7.2": {  ## removed 0.5 from output, no hinge loss, small n_vectors, n_kernel
    ...
    "tv7.3": {  ## Removed 0.5 from output and using hinge loss, OHE
        "n_vectors": (10, 600, 'int'),
        "n_kernel": (10, 300, 'int'),
        "kernel_std": (0.2, 1.0, 'float'),
        "use_kernel": (['rbf', 'lin'], 'categorical'),
        "use_score": (['lin', 'rbf'], 'categorical'),
        "normalize_proj_diff": 'max-detached', # 'max/n_kernel', 'max-detach'
        "detached_vec_norm": True,
    ...
    "tv7.4": {  ## Removed 0.5 + hinge loss + embd + small kernel_std
        "n_vectors": (5, 400, 'int'),
        "n_kernel": (5, 300, 'int'),
        "kernel_std": (0.1, 0.8, 'float'),
        "use_kernel": (['rbf', 'lin'], 'categorical'),
        "use_score": (['lin', 'rbf'], 'categorical'),
        "normalize_proj_diff": 'max-detached', # 'max/n_kernel', 'max-detach'
        "detached_vec_norm": True,
...
tv_kernel_parameters = {
    'rbf': {
        "gamma": (['auto', 0.01, 0.1, 1.0, 10], 'categorical')
    },
    'lin': {}
}
d_embedding_dicts = {
    ...
    "tv7.2": (64, 512, 'int'),
    "tv7.3": 64,
    "tv7.4": (64, 128, 'int'), # reduced from 512 to fit some datasets in memory
}
...
def get_model_training_params(trial):
    ...
    #     model_args.update(tv_kernel_parameters[model_args['use_kernel']])
    training_args = {
        'batch_size': batch_size,
