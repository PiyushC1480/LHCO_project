...
from lib import DataConfig, data_preproc, prepare_tensors, make_optimizer, calculate_metrics, get_openml_db
# from lib import BIN_CHECKPOINT as CHECKPOINT_DIR
# from bin import MLP, AutoInt, DCNv2, SAINT, TV6

def get_training_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", type=str, required=True, choices=['tv6', 'mlp', 'autoint', 'dcnv2', 'saint'])
    ...
    parser.add_argument("--task", type=str, choices=['binclass', 'openml', 'regression', 'multiclass'], required=True)
...
def get_dataset_details(reduce_data, model='tv7.4', task='binclass', dataset='train_1748_Sales_DataSet_of_SuperMarket', batch_size=64, early_stop=16):
    """args"""
    device = torch.device('cuda')
    args = Namespace(
        model =model,
        output = 'configs/tuned',
        dataset = dataset,
        task = task,
    ...
    preproc_type = 'ftt'
    if args.task == 'binclass':
        CHECKPOINT_DIR = '/home/dell/trainable_vectors/tabel_experiments/tp-berta/checkpoints/tp-bin'
        FINETUNE_DATA = '/home/dell/trainable_vectors/tabel_experiments/tp-berta/data/finetune-bin'
    elif args.task == 'openml':
        CHECKPOINT_DIR = '/home/dell/trainable_vectors/tabel_experiments/tp-berta/checkpoints/openml'
        FINETUNE_DATA = '/home/dell/trainable_vectors/tabel_experiments/tp-berta/data/finetune-openml/'
    # CHECKPOINT_DIR = '/home/dell/trainable_vectors/tabel_experiments/tp-berta/checkpoint/tp-bin' 
    if args.task == 'openml':
    ...
    details = {'dataset': dataset, 'train': len(train_loader.dataset), 'val': len(val_loader.dataset), 'test': len(test_loader.dataset)}
    return details

###########################################################################################
# Later part is not needed so commenting them
###########################################################################################
...
#     )
#     # if args.model == 'tv6':
#     # loss_fn = lambda y,y_hat: F.hinge_embedding_loss(y, y_hat, margin=0.1)
# 
#     """Utils Function"""
