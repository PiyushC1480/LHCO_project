...
def get_training_args():
    ...
    parser.add_argument("--output", type=str, default='finetune_param_effect')
    ...
    parser.add_argument("--reduce_data", type=float, default=0.8)
    parser.add_argument("--param_name", type=str, required = True)
    parser.add_argument("--param_ratio", type=float, default =0.5)
    args = parser.parse_args()

    # args.output = f'{args.output}/{args.task}/{args.model}-tuned/{args.dataset}'
    args.file_name = f'{args.param_name}-{args.param_ratio}-tuned.json'
    ...
    cfg['model'][args.param_name] = int(cfg['model'][args.param_name]* args.param_ratio) # selecting param_ratio of total quantity
    if cfg['model'][args.param_name] == 0:
        cfg['model'][args.param_name] += 1
...
def train_predict(model, optimizer):
    ...
    def evaluate(parts):
        ...
        for part in parts:
            ...
            for batch in dataloaders[part]:
                ...
                losses[part]+= loss_fn(pred,y).cpu().item()
                total_time[part] += t1
            ...
            scores[part] = score
        return scores, losses, total_time
    ...
    for epoch in range(500):
        ...
        scores, losses, total_times = evaluate(['train', 'val', 'test'])
        val_task_losses.append(losses['val']/(iteration + 1))
        test_task_losses.append(losses['test']/(iteration + 1))
    ...
    _,_,total_times = evaluate(['val', 'test'])
...
tr_task_losses, val_task_losses, test_task_losses, ev_metrics, test_metrics, best_metric, final_test_metric, total_times,  param_total_size, param_total_count, model_size = \
    train_predict(model, optimizer)

save_result(
    args,
    best_ev=best_metric, final_test=final_test_metric,
    tr_losses=tr_task_losses, val_losses=val_task_losses,
    test_losses=test_task_losses, ev_metrics=ev_metrics,
