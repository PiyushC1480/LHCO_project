...
class Dataset:
    ...
    def build_X(
        ...
        for column_idx in range(C[test_idx].shape[1]):
            # NOTE: This does not assign value to RHS, no idea why
            # unk_bool_idx = C[test_idx][:,column_idx] == unknown_value
            # C[test_idx][unk_bool_idx, column_idx] = max_values[column_idx] + 1

            unk_bool_idx = C[:,column_idx] == unknown_value
            C[unk_bool_idx, column_idx] = max_values[column_idx] + 1
