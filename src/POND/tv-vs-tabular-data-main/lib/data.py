...
class Dataset2:
    ...
    cat_size: Optional[List[int]] = None
    ...
    def from_csv(
        ...
        if len(X_str_idx) > 0: # for LM baselines (e.g., TP-BERTa), all categorical features are treated as strings
            ...
            X_str = {'train': df.iloc[:, X_str_idx].values.astype(np.str_).astype(np.object_)}
    ...
    def get_category_sizes(self, part: str) -> List[int]:
        if part == 'all':
            ...
            #     li[p] = get_category_sizes(self.X_cat[p])
            X= np.vstack([self.X_cat[p] for p in ['train', 'val', 'test']])
            cats = get_category_sizes(X)

            # cats = li['train']
            # cats = np.max([cats, li['val']], axis =0)
            # cats = np.max([cats, li['test']], axis = 0)
            return list(cats)
