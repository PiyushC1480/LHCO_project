...
def split(dataset: Dataset2, config: DataConfig = None):
    ...
    # sample_idx, _ = train_test_split(sample_idx, test_size=config.reduce_data, random_state=42, stratify=stratify)
    
    if config and config.pre_train or 'test' in dataset.y:
        test_idx = None # no need for test split
        # Just remove reduce_data % of points from training data
        if config.reduce_data > 0:
            train_idx, _ = train_test_split(
                sample_idx,
                test_size=config.reduce_data,
                random_state=42,
                stratify=stratify
            )
...
def data_preproc(dataset_name, config: DataConfig = None, no_str=False, tt:str = None) -> Dataset2:
    ...
    cache_dir = (
        ...
        else config.data_dir / f'cache-{config.reduce_data}' / config.preproc_type
