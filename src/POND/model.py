...
KERNEL_TYPE = ty.Literal['lin', 'rbf'] #'poly'
...
class TV7(nn.Module):
    def __init__(
        ...
        reg_wt: float = 0.1,  ##1e-4
        train_std = True,
        gamma: float = 1.0,  #kernel param
        d: float = 2,
        s: float = 1,
        c: float = 0,
        ...
        combine_type: ty.Optional[ty.Literal['embd', 'default', 'ohe']] = 'default',
    ) -> None:
        '''
        # Parameters
        '''
    ...
    def init_phase2(self, x_num, x_cat, y, n_kernel):
        ...
        print(f"n_pivot: {self.n_pivot}, n_pos_pivot: {self.n_pos_pivot}, n_neg_pivot: {self.n_neg_pivot}")
    ...
    def get_score(self, proj_point, proj_pivot, pivot_std, pos_pivot: bool, score_weight = None):
        ...
        self.max_proj_diff = (self.max_proj_diff_p, self.max_proj_diff_n)
        if pos_pivot: # picking 0 for +ve points
            max_proj_diff_idx = 0
        else: # picking 1 for -ve points
            max_proj_diff_idx = 1
    ...
    def regularizer(self, use_low_gpu = True):
        ...
        if use_low_gpu:
            ...
            try:  # Faster method for smaller number of vectors
                ...
                # Using for loop instead for huge number of vectors (low gpu utilization)
                dot_prod_mat = 0
                for j in range(self.vectors.shape[0]):
                    dot_prod_mat += (F.cosine_similarity(
                                self.vectors[j].unsqueeze(dim = 0),
